...
Calendar.prototype = {
    ...
    monthInfo : function monthInfo(year, month, dayFormatter, Json) {
        // JSON example {'days':[{date:'140218',aveMood:'3',eventNum:'6'},...]}
        ...
        do {
            for (i=0; i<7; i++) {
                tempWeeks.push(dayFormatter ? dayFormatter(date) : date);
                date = new Date(date.getTime());
                date.setDate(date.getDate() + 1);
            }
            // console.log(date);
        ...
        for (i=0; i<tempWeeks.length; i++) {
            if (j < Json.days.length) {
                if (found === false) {
                    if (tempWeeks[i] != 0) {
                        found = true;
                        j = 0;
                        days.push(Json.days[j]);
                        j++;
                    } else {
                        days.push(b);
                    }
    ...
    monthDays : function monthDays(year, month) {
        console.log('monthDays is called!');
    ...
    monthJSON : function monthJSON(year, month, Json) {
        var getDayOrZero = function getDayOrZero(date) {
            return date.getMonth() === month ? date.getDate() : 0;
        };
        return this.monthInfo(year,month,getDayOrZero, Json);
    },
